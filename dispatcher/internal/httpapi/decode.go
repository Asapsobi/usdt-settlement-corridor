...
// urlParamInt64 is urlParam plus base-10 integer parsing, for {order_id}.
...
// urlParamInt is urlParamInt64 narrowed to int, for {id} (a slot id).
func urlParamInt(w http.ResponseWriter, r *http.Request, key string) (int, bool) {
	id, ok := urlParamInt64(w, r, key)
	if !ok {
		return 0, false
	}
	return int(id), true
