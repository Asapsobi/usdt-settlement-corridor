// Package db owns C5's own Postgres connection pool and transaction
// helper. C5 has its own database, separate from every other
// component's -- same posture as every prior component's own internal/db.
package db
...
// ConfigFromEnv reads Config from DISPATCHER_DATABASE_URL. It is the
// only environment variable this package reads.
func ConfigFromEnv() (Config, error) {
	url := os.Getenv("DISPATCHER_DATABASE_URL")
	if url == "" {
		return Config{}, errors.New("db: DISPATCHER_DATABASE_URL is not set")
	}
