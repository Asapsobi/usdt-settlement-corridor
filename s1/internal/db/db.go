// Package db owns S1's own Postgres connection pool and transaction
// helper. S1 has its own database, separate from every other
// component's -- it is a standalone service C5 calls over HTTP like
// every other component does, same posture as every prior component's
// own internal/db.
package db
...
// ConfigFromEnv reads Config from S1_DATABASE_URL. It is the only
// environment variable this package reads.
func ConfigFromEnv() (Config, error) {
	url := os.Getenv("S1_DATABASE_URL")
	if url == "" {
		return Config{}, errors.New("db: S1_DATABASE_URL is not set")
	}
