// Package db owns C3's own Postgres connection pool and transaction
// helper. C3 has its own database, separate from C1's and C2's -- it is
// a standalone service that calls C1 over HTTP like every other
// component does, same posture as C2's own internal/db.
package db
...
// inside a caller's transaction. Same shape as C1's and C2's own Queryer,
// deliberately -- not shared across modules since these are three
...
// ConfigFromEnv reads Config from SCREENING_DATABASE_URL. It is the only
// environment variable this package reads.
func ConfigFromEnv() (Config, error) {
	url := os.Getenv("SCREENING_DATABASE_URL")
	if url == "" {
		return Config{}, errors.New("db: SCREENING_DATABASE_URL is not set")
	}
