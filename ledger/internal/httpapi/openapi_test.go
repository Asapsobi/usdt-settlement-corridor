...
func loadSpec(t *testing.T) openAPISpec {
	...
	// Parsing rather than substring-matching is the point: the spec is a
	// published contract that C2/C4/C6 will be written against, quite
	// possibly by feeding it to a code generator. A substring check
	// passes happily on a file no YAML parser will accept -- which is
	// exactly what had happened here, an unquoted "slot: for" in a
	// description that made the whole document unparseable while this
	// test stayed green.
...
func TestEveryRouteIsInOpenAPISpec(t *testing.T) {
	...
	routes := map[string][]string{
		...
		"/v1/reconciliation/snapshots":         {"post"},
		"/v1/system/halt":                      {"get", "post"},
