// Package money is the proof-run driver's minimal money primitive,
// mirroring every prior component's own internal/money exactly: signed
// minor units in an int64, never a float, 6 decimal places (USDT's own
// precision on both BEP20 and TRC20). Duplicated, not shared -- separate
// Go modules, no common internal package between them, the same
// convention this whole project uses everywhere else.
package money
