// Package db owns C6's own Postgres connection pool and transaction
// helper. C6 has its own database, separate from every other
...
// ConfigFromEnv reads Config from GATEWAY_DATABASE_URL. It is the only
// environment variable this package reads.
func ConfigFromEnv() (Config, error) {
	url := os.Getenv("GATEWAY_DATABASE_URL")
	if url == "" {
		return Config{}, errors.New("db: GATEWAY_DATABASE_URL is not set")
	}
