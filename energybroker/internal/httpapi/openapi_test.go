...
func loadSpec(t *testing.T) openAPISpec {
	...
	// Parsing rather than substring-matching, same reasoning as every
	// sibling module's own equivalent test: the spec is a published
	// contract, quite possibly fed to a code generator one day, and a
	// substring check passes happily on a file no YAML parser would
	// accept.
